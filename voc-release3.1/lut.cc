...
void *process(void *thread_arg) {
  ...
  int *A_src = A; 
  double *B_src = B; 
  int feat_off = B_dims[0]*B_dims[1]; 
  for (int x = 0; x < C_dims[1]; ++x){
    for (int y = 0; y < C_dims[0]; ++y){
      double val = 0; 
      for (int xp = 0; xp < B_dims[1]; xp++){
        int *A_off = A_src + (x+xp)*A_dims[0] + y;
        double *B_off = B_src + xp*B_dims[0];
        for (int yp = 0; yp < B_dims[0]; yp++){
          //val += *(A_off++) * *(B_off++);
          //int offset = (*A_off - 1)*feat_off;
          //val += *(B_off+offset); 
          val = val + *(B_off + ((*A_off) - 1)*feat_off);
          A_off++; B_off++; 
        }
      }
      *(dst++) += val;
  ...
  /*
  for (int f = 0; f < num_features; f++) {    // For every feature dimension
    double *dst = C;
    double *A_src = A + f*A_dims[0]*A_dims[1];    // The head of A of feature dimension f
    double *B_src = B + f*B_dims[0]*B_dims[1];    // The head of B of feature dimension f
    for (int x = 0; x < C_dims[1]; x++) {       // Iterate over x dimension of C
      for (int y = 0; y < C_dims[0]; y++) {     // Iterate over y dimension of C
	double val = 0;
	for (int xp = 0; xp < B_dims[1]; xp++) {    // Use B to convolve A
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  if (mxGetNumberOfDimensions(mxA) != 2 || 
      mxGetClassID(mxA) != mxINT32_CLASS)
