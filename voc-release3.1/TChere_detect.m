function [boxes] = TChere_detect(input, model, thresh, bbox, ...
...
for i = 1:length(model.rootfilters)
  rootfilters{i} = double(model.rootlut{i}); 
end
partfilters = [];
for i = 1:length(model.partfilters)
  partfilters{i} = double(model.partfilters{i}.wlut); 
end
...
save_kmean_idx = cell(length(feat),1);
...
for level = interval+1:length(feat)
  ...
  [fs1 fs2 fs3] = size(featr);
  featr = reshape(featr, fs1 * fs2, fs3);
  
  if model.method == 'kmean'
      [kmean_idx d] = vgg_nearest_neighbour(featr', model.center); 
      % kmean_idx = int32(flann_search(model.index,single(featr'),1,model.parameters))';
  elseif model.method == 'hier_kmean'
      kmean_idx = vgg_nearest_neighbour_hierarchical(featr', model.root_center, model.center); 
  end
  
  kmean_idx = reshape(kmean_idx, [fs1 fs2]);    
  rootmatch = lut(kmean_idx, rootfilters, 1, length(rootfilters));  
  save_kmean_idx{level} = padarray(kmean_idx, [pady padx 0], kmean_idx(1,1));

  if length(partfilters) > 0
     
     if isempty(save_kmean_idx{level-interval})
         featp = padarray(feat{level-interval}, [2*pady 2*padx 0], 0); 

        [fs1 fs2 fs3] = size(featp);
        featp = reshape(featp, fs1 * fs2, fs3); 
        
        if model.method == 'kmean'
            [kmean_idx d] = vgg_nearest_neighbour(featp', model.center); 
            % kmean_idx = int32(flann_search(model.index,single(featp'),1,model.parameters))';
        elseif model.method == 'hier_kmean'
            kmean_idx = vgg_nearest_neighbour_hierarchical(featp', model.root_center, model.center); 
        end
        
        kmean_idx = reshape(kmean_idx, [fs1 fs2]);    
        partmatch = lut(kmean_idx, partfilters, 1, length(partfilters));
        save_kmean_idx{level-interval} = kmean_idx;
        
     else 
        partmatch = lut(save_kmean_idx{level-interval}, partfilters, 1, length(partfilters));
     end
