function model = TChere_train(ex_para, name, model, pos, neg, warp, randneg, iter, ...
...
for t = 1:iter
  ...
  if warp > 0
    ...
    [numadded, numpositives] = poslatent(name, t, model, pos, overlap, fid, ex_para);
  end
  num = num + numadded;
  fclose(fid);
...
% we create virtual examples by flipping each image left to right
function [num, numpositives] = poslatent(name, t, model, pos, overlap, fid, ex_para)
numpos = length(pos);
model.interval = ex_para.sinter; % model.interval = 5;
model.octave = ex_para.soctaves; 
...
for i = 1:numpos
  ...
  if bbox(3)>size(im,2) || bbox(4)>size(im,1)
      fprintf('BAD BBOX%s: iter %d: latent positive: %d/%d\n', name, t, i, numpos);
      bbox
      continue;
  end
  [im, bbox] = croppos(im, bbox); 
  % figure(1), imshow(im);
  box = detect(im, model, -1, bbox, overlap, 1, fid, 2*i-1);
  if ~isempty(box)
    c = box(1,end-1);
  ...
  box = TChere_detect(im, model, 0, bbox, overlap, 1, fid, 2*i);
  if ~isempty(box)
    c = box(1,end-1);
    numpositives(c) = numpositives(c)+1;
    num = num+1;
    %figure(1), subplot 122, showboxes(im, box);
  end
end
...
for i = 1:numneg
  ...
  if size(feat,2) > rsize(2) && size(feat,1) > rsize(1)
    for j = 1:rndneg
      ...
      x = ceil(rand(1)* (size(feat,2)-rsize(2)+1));
      y = ceil(rand(1)* (size(feat,1)-rsize(1)+1));
